...
RegisterCommand('anon', function(source, args, rawCommand)
	...
	TriggerClientEvent('chat:addMessage', -1, { args = { _U('anon_prefix', 'Anonimo'), args }, color = { 0, 153, 204 } })
	--print(('%s: %s'):format(args))
end, false)

RegisterCommand('alt', function(source, args, rawCommand)
	if source == 0 then
		print('esx_rpchat: you can\'t use this command from rcon!')
		return
	end
	...
	TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, source, _U('alt_prefix', name), args, { 0, 0, 255 })
	--print(('%s: %s'):format(name, args))
end, false)
